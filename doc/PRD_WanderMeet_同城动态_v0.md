...
| 阶段 | 范围 | 说明 |
|------|------|------|
| **V0.5 活动态** | 活动参与者、活动结束后 72h 内 | 图文动态挂在活动下，不进入全城广场 |
...
迁移：`alembic/versions/20260525_0023_city_feed.py`（`posts`、`post_likes`、`post_comments`、`user_follows`）。
...
- 活动态：须 `joined` 报名；非城市大群活动；取消活动不可发；时间窗为结束后 72h（无 `end_at` 时用 start+2h 起算）。
- 同城：须 `cityCode`；每日发帖上限；联系方式过滤；拉黑用户内容对 viewer 不可见。
- 图片：须为本服务 BOS 已存 URL（`validate_stored_feed_image_url`）。

部署：新环境需 `alembic upgrade head`（含 `0022` 增长信任 + `0023` feed）。
...
2. 发布同城动态：选图 → `POST /me/feed/images` → `POST /feed/posts`。
3. 活动态：已报名活动 → 活动详情「发一条」→ `POST /activities/{id}/posts`（注意时间窗）。
