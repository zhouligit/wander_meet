...
### `POST /api/v1/wm/auth/email/forgot-password`

向**已注册且为邮箱密码账号**的邮箱发送 6 位验证码。为防邮箱枚举，**无论邮箱是否存在**均返回成功。
...
| 字段 | 类型 | 说明 |
| --- | --- | --- |
| expireInSeconds | int | 验证码有效秒数（默认 900） |
...
| 400 | 邮箱格式错误 |
| 429 | IP 限流或同邮箱发信过于频繁 |
| 502 | SMTP 发送失败（仅真实发信且已命中注册用户时） |

---

## 2.3.2 忘记密码 — 重置密码（H5）

### `POST /api/v1/wm/auth/email/reset-password`

校验邮件验证码并设置新密码；成功返回与 §2 相同的登录 `data`，并吊销该用户全部 refresh。
...
| --- | --- | --- | --- | --- |
| email | body | string | 是 | 邮箱 |
| code | body | string | 是 | 邮件中的 6 位验证码 |
| newPassword | body | string | 是 | 新密码，规则同注册 |

### 响应 `data`

与 §2 相同（`accessToken`、`refreshToken`、`user`）。

### 错误

| HTTP | 说明 |
| --- | --- |
| 400 | 邮箱/验证码/密码格式错误，或验证码无效 |
| 403 | 用户封禁 / 限制 |
| 429 | IP 限流 |
